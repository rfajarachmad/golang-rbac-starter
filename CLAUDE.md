...
- **`internal/entity/`** — GORM database models (User, Contact, Address, Role, Permission). Define table names and relationships.
...
- **`internal/delivery/http/middleware/`** — Auth middleware validates `Authorization` header token via `UserUseCase.Verify()`, loads role + permissions, stores `model.Auth` in `ctx.Locals("auth")`. `RequirePermission()` and `RequireRole()` middleware enforce per-route authorization.
...
- **RBAC**: Three roles (`admin`, `user`, `viewer`) with table-based permissions. Auth middleware loads role + permissions per request; `RequirePermission()` middleware enforces per-route access. Permissions follow `resource:action` naming (e.g., `contact:create`, `admin:user:list`).
- **Scoped data**: Contacts belong to a user, addresses belong to a contact. All queries enforce ownership via `user_id`/`contact_id`. Admin endpoints (`/api/admin/*`) bypass ownership scoping.
...
- `test/helper_test.go` — `ClearAll()`, `SeedUser()`, `SeedUserWithRole()`, `SeedContact()`, `SeedAddress()` helpers for test data setup.
- `test/user_test.go`, `test/contact_test.go`, `test/address_test.go` — Test files per domain.
- `test/rbac_test.go` — RBAC integration tests (permission enforcement, admin endpoints, role assignment).
...
- **Admin only**: `GET/DELETE /api/admin/users/:userId`, `GET /api/admin/users`, `PATCH /api/admin/users/:userId/role`, `GET /api/admin/roles`, `GET /api/admin/roles/:roleId`

## Database

PostgreSQL with GORM. Migrations in `db/migrations/` using golang-migrate (sequential numbering: `000001_`, `000002_`, etc.).

Six tables: `users` (serial PK, FK to roles), `contacts` (UUID PK, FK to users), `addresses` (UUID PK, FK to contacts), `roles` (serial PK), `permissions` (serial PK), `role_permissions` (join table).

Default admin user seeded by migration: `admin@example.com` / `admin123`.
